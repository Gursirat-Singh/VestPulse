...
exports.mergeFinancialMetrics = mergeFinancialMetrics;
...
function isValidValue(val, isNumericFieldWithZeroInvalid = false) {
    ...
    if (typeof val === "object" && Object.keys(val).length === 0)
        return false;
    if (isNumericFieldWithZeroInvalid && typeof val === "number" && val === 0)
        return false;
    return true;
}
// Helper to merge string fields by priority order
function getMergedString(pList, selector) {
...
function getMergedNumeric(pList, selector, isZeroInvalid = false) {
    for (const p of pList) {
        const val = selector(p);
        if (isValidValue(val, isZeroInvalid))
            return val;
    }
    return 0;
}
function mergeFinancialMetrics(...providers) {
    const pList = providers.filter(Boolean);
    // 1. Merge Profile
    const profile = {
    ...
    pList.forEach(p => {
        if (p.sources)
            p.sources.forEach(src => sourcesSet.add(src));
    });
