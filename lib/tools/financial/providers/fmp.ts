...
export class FmpProvider implements FinancialProvider {
  ...
  async fetchData(ticker: string): Promise<Partial<FinancialData> | null> {
    ...
    try {
      ...
      let ratiosRaw: any = null;
      let estimatesRaw: any = null;
      ...
      if (Array.isArray(pJson) && pJson.length > 0) profileRaw = pJson[0];

      const iJson = await extractJson(incomeResult);
      if (Array.isArray(iJson)) incomeRaw = iJson;

      const cfJson = await extractJson(cashFlowResult);
      if (Array.isArray(cfJson)) cashFlowRaw = cfJson;

      const mJson = await extractJson(metricsResult);
      if (Array.isArray(mJson) && mJson.length > 0) metricsRaw = mJson[0];

      const rJson = await extractJson(ratiosResult);
      if (Array.isArray(rJson) && rJson.length > 0) ratiosRaw = rJson[0];

      const eJson = await extractJson(estimatesResult);
      if (Array.isArray(eJson) && eJson.length > 0) estimatesRaw = eJson[0];
      ...
      const historicalMap: Record<string, Partial<HistoricalFinancial>> = {};

      incomeRaw.forEach((inc: any) => {
      ...
      const historical: HistoricalFinancial[] = Object.values(historicalMap)
        .map((item) => item as HistoricalFinancial)
      ...
      const parsedEstimates: AnalystEstimate = {
      ...
      if (profileRaw) sources.push(`https://financialmodelingprep.com/stable/profile?symbol=${cleanTicker}`);
      if (incomeRaw.length > 0) sources.push(`https://financialmodelingprep.com/stable/income-statement?symbol=${cleanTicker}`);
      if (cashFlowRaw.length > 0) sources.push(`https://financialmodelingprep.com/stable/cash-flow-statement?symbol=${cleanTicker}`);
      if (metricsRaw) sources.push(`https://financialmodelingprep.com/stable/key-metrics-ttm?symbol=${cleanTicker}`);
      if (ratiosRaw) sources.push(`https://financialmodelingprep.com/stable/ratios-ttm?symbol=${cleanTicker}`);
